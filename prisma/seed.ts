...
async function main() {
  ...
  for (let i = 0; i < 12; i++) {
    const lead = await prisma.lead.create({
      data: {
        ...
        source: leadSources[i] as any,
        stage: leadStages[i] as any,
        priority: leadPriorities[i] as any,
        score: 40 + (i * 5),
  ...
  // Demo Activities for first few leads
  for (let i = 0; i < 4; i++) {
    await prisma.leadActivity.create({
      data: {
        leadId: leads[i].id,
        userId: admin.id,
        type: ["CALL", "WHATSAPP", "EMAIL", "SITE_VISIT"][i] as any,
